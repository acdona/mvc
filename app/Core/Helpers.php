...
function toEncode(string $string, string $from = "UTF-8", string $toEncode = "ISO-8859-1"): string
{
    return iconv($from, $toEncode, $string);
}
 ...
 * @return string|bool
 */
function is_email(string $email): string|bool
{
...
function anti_injection(string $sql): string
{
...
function str_slug(string $string): string
{
    ...
    $slug = str_replace(
        ...
        str_replace(
            " ",
            "-",
            trim(strtr(utf8_decodeBR($string), utf8_decodeBR($formats), $replace))
        )
    );
    return $slug;
}
...
{
    return number_format((!empty($price) ? (float) $price : 0), 2, ",", ".");
}
...
/**
 * Método criado para substituir o utf8_decode
 * @param string $string
 * @return string
 */
function utf8_decodeBR(string $string): string
{
...
/**
 * @return null|string|array|Source\Support\Message
 */
function flash(): null|string|array|Source\Support\Message
{
    $session = new \Source\Core\Session();
